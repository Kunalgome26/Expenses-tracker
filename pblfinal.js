...
function setTotalAmount() {
    ...
    // Add initial budget as a transaction
...
function addTransaction(e) {
    ...
    const transaction = {
        id: Math.floor(Math.random() * 1000000),
        text: desc,
        amount: -Math.abs(amt), // Expenses are always negative
        date
    };
...
function addSaving() {
    ...
    const transaction = {
        id: Math.floor(Math.random() * 1000000),
        text: "Savings Added",
        amount: +amt, // Savings are positive
        date: today
    };
...
function updateValues() {
    ...
    // Calculate income (sum of positive transactions)
    const income = amounts
        .filter(item => item > 0)
        .reduce((acc, item) => acc + item, 0)
        .toFixed(2);
    
    // Calculate expenses (sum of negative transactions)
